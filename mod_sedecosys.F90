...
!!!=== ver 2014/01/20   Copyright (c) 2013-2014 Takashi NAKAMURA  =====
...
!          output parameters
     &            ,sed_Pg         &   ! Gross photosynthetic rate (nmol cm-2 s-1)
     &            ,sed_R          &   ! Respiration rate (nmol cm-2 s-1)
     &            ,sed_Gn         &   ! Calcification rate (nmol cm-2 s-1)
...
! output parameters
      real(8), intent(out) :: sed_Pg 
      real(8), intent(out) :: sed_R  
      real(8), intent(out) :: sed_Gn 
...
!!!------------Set parameters  ----------------------------------
      real(8), parameter :: dz = 0.1d0  ! (cm) grid tickness
...
!---- Assimilation ratio --------------------------
      real(8), parameter :: r_assim =  0.5d0       !(no dim.)  !!!‚Ä‚«‚Æ‚¤
!---- Aerobic respiration --------------------------
      real(8), parameter :: Rp_max =  1.0d0     !(nmol cm-3 s-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Krp_POC = 1.0d0     !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Krp_DO =  10.0d0     !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Rd_max =  5.0d0     !(nmol cm-3 s-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Krd_DOC = 120.0d0    !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Krd_DO =  10.0d0     !(umol L-1)  !!!‚Ä‚«‚Æ‚¤

      real(8), parameter :: Rdoc_max =  1.0d-3     !(nmol cm-3 s-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Krdoc_DO =  10.0d0     !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
!---- Decomposition --------------------------
      real(8), parameter :: Dp2d_max =1.0d-1     !(nmol cm-3 s-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kp2d_POC = 10.0d0     !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
#if defined NUTRIENTS
!---- Nitrification --------------------------
!      NH4+ + (3/2)O2 -> NO2- + 2H2O + 2H+
      real(8), parameter :: Nit1_max =  5.0d-1 !!!10.0d0  !(nmol cm-3 s-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kn1_NH4 =  250.0d0   !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kn1_DO =  10.0d0   !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
!      NO2- + (1/2)O2 -> NO3-
      real(8), parameter :: Nit2_max =  5.0d-1 !!!10.0d0  !(nmol cm-3 s-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kn2_NO2 =  250.0d0   !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kn2_DO =  10.0d0   !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
!---- Denitrification --------------------------
!      NO3- + H+ +(5/4)CH2O -> (1/2)N2 + (5/4)CO2 + (7/4)H2O
      real(8), parameter :: DNd_max =  1.0d-2 !!!10.0d0  !(nmol cm-3 s-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kdnd_DOC = 100.0d0   !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kdnd_NO3 =  250.0d0   !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kdnd_DO =  10.0d0   !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: DNp_max =  1.0d-2 !!!10.0d0  !(nmol cm-3 s-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kdnp_POC =  5.0d0   !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kdnp_NO3 = 250.0d0   !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
      real(8), parameter :: Kdnp_DO =  10.0d0   !(umol L-1)  !!!‚Ä‚«‚Æ‚¤
!---- C:N:P ratio --------------------------
      ...
      real(8) :: Diff, t_dbl
#if defined NUTRIENTS
      real(8) :: NH4ass(Nsed), NO2ass(Nsed), NO3ass(Nsed)
      real(8) :: PO4ass(Nsed)
      real(8) :: V_NH4, V_NO3, V_PO4
      real(8) :: Nit1(Nsed), Nit2(Nsed)
...
!---- Sediment column fluxes (nmol cm-2 s-1) --------------------------
      ...
      DO k=1,Nsed-1
        Diff = 1.0d-4 !!!   !(cm2 s-1) Diffusion coefficient by Yamamoto
...
!    DON (umol L-1)
     ...
     &        +((NH4ass(k)+NO2ass(k)+NO3ass(k))*(1.0d0 - r_assim)       &
     &          +Dp2don(k)                                              &
     &          -Rdon(k)                                                &
     &          -DNdon(k)                                                &
...
!    PON (umol L-1)
     ...
     &        +((NH4ass(k)+NO2ass(k)+NO3ass(k)) * r_assim               &
     &          -Dp2don(k)                                              &
     &          -Rpon(k)                                                &
     &          -DNpon(k)                                                &
...
!!!---- Set output parameters --------------------------
      ...
      DO k=1,Nsed
        sed_Pg = sed_Pg + Pg(k)*dz      
        sed_R  = sed_R  + (Rdoc(k)+Rpoc(k)) *dz       
        sed_Gn = sed_Gn + Gn(k) *dz             
      ENDDO
...
#  endif
# endif
        write(71,*) time/86400.d0, 0.0d0, Pg
        write(72,*) time/86400.d0, 0.0d0, Rdoc
        write(73,*) time/86400.d0, 0.0d0, Rpoc
        write(74,*) time/86400.d0, 0.0d0, Gn
#if defined NUTRIENTS
        write(75,*) time/86400.d0, 0.0d0, Nit1
