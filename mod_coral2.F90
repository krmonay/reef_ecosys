...
!!!=== ver 2013/12/03   Copyright (c) 2012-2014 Takashi NAKAMURA  =====
...
!!   Coral minimum quota
!      data QC0coral /286.d0  /   ! Minimum quota (umol cm-2)   (Tanaka et al. 2010)  ＊minimumかどうかは分からない。
...
!        coral internal conditions
            ...
            CORAL(ng)%QC (n,i,j)=15.d0 !450 ug/cm2 = 15 umol cm-2 -> 5.e6 umol kg-1 (腔腸内平均濃度とした）
!            CORAL(ng)%QC (n,i,j)=285.d0 ! Tanaka et al. 2010
...
!          CORAL(ng)%Q13C (n,i,j)=R13C/(1.+R13C)*CORAL(ng)%QC(n,i,j)
            CORAL(ng)%Q13C (n,i,j)=R13C*CORAL(ng)%QC(n,i,j)

            R13C=R13C_fromd13C(0.d0)
!          CORAL(ng)%DI13Ccoe (n,i,j)=R13C/(1.+R13C)*CORAL(ng)%DICcoe (n,i,j)
            CORAL(ng)%DI13Ccoe (n,i,j)=R13C*CORAL(ng)%DICcoe (n,i,j)

            R13C=R13C_fromd13C(0.d0)
!          CORAL(ng)%DI13Ccal (n,i,j)=R13C/(1.+R13C)*CORAL(ng)%DICcal (n,i,j)
...
# endif
# if defined CORAL_NUTRIENTS
            ZOOX(ng)%NO3(n,i,j) = 0.0d0  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
            ZOOX(ng)%NO2(n,i,j) = 0.0d0  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
            ZOOX(ng)%NH4(n,i,j) = 0.0d0  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
            ZOOX(ng)%PO4(n,i,j) = 0.0d0  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
...
!     &    *CORAL(ng)%DOcoe(n,i,j)/(K_DO(n)+CORAL(ng)%DOcoe(n,i,j))
      ...
      real(8), parameter :: Rmax(Ncl) = 0.53d0 !!!0.53d0*  0.37d0** !Maximum respiration rate (nmol O2 cm-2 s-1) *Tuned using Kuhl et al (1995), Al-Horani et al. (2003) data
      real(8), parameter :: K_QC(Ncl) = 5.d0   !umol cm-2!!!*適当
      real(8), parameter :: K_DO(Ncl) = 46.d0  ! ca. 46 (umol kg-1) Newton & Atkinson (1991)

      real(8), parameter :: ratio(Ncl) = 1.0d0 !!!0.9d0** 0.8d0  !1.0d0* !!!*適当
...
!      real(8), parameter :: k_Gn(Ncl) /1.d-2/!(未使用）
      ...
      real(8), parameter :: eeff   = 0.3d0  !!! 0.01d0** 0.02d0 !0.3d0*!Energy efficency of calcification
      real(8), parameter :: E_other= 5.0d3  !!! 1.0d4** ! 5.0d3* Energy flux of other metabolisms (nJ cm-2 s-1)

      real(8), parameter :: k_CO2i = 1.5d-3 ! 1.5d-3* !permeability coefficient (cm s-1): Sueltemeyer and Rinast (1996): (1.49d-3 cm s-1)
      real(8), parameter :: k_TA   = 3.0d-3 !!! 3.0d-5**  5.0d-5 !3.0d-3* !conductivity of TA through the leak pass (cm s-1) *Tuned
      real(8), parameter :: k_DIC  = 3.0d-3 !!! 3.0d-5**  5.0d-5 !3.0d-3* !conductivity of DIC through the leak pass (cm s-1) *Tuned

#if defined CORAL_CARBON_ISOTOPE
!----- for carbon isotope ------------------------
      real(8), parameter :: a_phot = -20.0d-3 + 1.0d0!!!!!!!!!!!!!!!!!!適当
...
!----- Coral minimum quota ------------------------
!      real(8), parameter :: QC0(Ncl) = 2.0d0    ! Minimum quota (umol cm-2)  !!!!!!!!!!!!!!!!!!適当
      real(8), parameter :: QC0(Ncl) = 200.0d0    ! Minimum quota (umol cm-2)  !!!!!!!!!!!!!!!!!!適当
# if defined CORAL_NUTRIENTS
      real(8), parameter :: QN0(Ncl) =  0.20d0    ! Minimum quota (umol cm-2)  
      real(8), parameter :: QP0(Ncl) =  0.02d0   ! Minimum quota (umol cm-2) 
# endif
#endif
#if defined CORAL_SIZE_DYNAMICS
      real(8), parameter :: g_max(Ncl) = 1.0d-2  ! Maximum growth rate (nmol cm-2 s-1)  !!!!!!!!!!!!!!!!!!適当!!!!!!!!!!!!!!
      real(8), parameter :: km_CNP = 1.0d-10     ! (s-1)!!!!!!!!!!!!!!!!!!適当!!!!!!!!!!!!!!
      real(8), parameter :: am_CNP = 5.0d0       ! (no dimension)!!!!!!!!!!!!!!!!!!適当!!!!!!!!!!!!!!
      real(8), parameter :: km_dam = 1.0d-10     ! (s-1)!!!!!!!!!!!!!!!!!!適当!!!!!!!!!!!!!!
#endif
...
!----- for CO2 system dynamics -------------------
      ...
      real(8) :: pHamb,fCO2amb,cCO2aqamb,cHCO3amb,cCO3amb,Wargamb  !!!!!不要：アウトプット用
#if defined CORAL_NONE_CO2_EQ
      real(8) :: cHcoe,cOHcoe
      real(8) :: cHcal,cOHcal
...
!---------- ROS related fluxes (nmol cm-2 s-1) ----------------------------

!     V_detoxは余剰の栄養素の関数
!
!      V_detox=k_detox(n)*exp(
...
!
!
!      F_detox=V_detox(n)*ROS(n,i,j)/(K_ROS(n)+ROS(n,i,j))
!      F_detox = 0.15d0 * CORAL(ng)%ROS(n,i,j) * hcoe  !(s-1) * (umol L-1) * (cm)*0.001(n/u)=(nmol cm-2 s-1)
             !0.15
!      F_detox=0.02*ROS(n,i,j)/(K_ROS(n)+ROS(n,i,j))
!      F_detox = 3.5d-2 * CORAL(ng)%ROS(n,i,j)/(1.0d-1 + CORAL(ng)%ROS(n,i,j))
      F_detox = 0.1d0 * CORAL(ng)%ROS(n,i,j)
    
!      F_damage=k_damage(n)*ROS(n,i,j)
      F_damage = 0.017d0 * CORAL(ng)%ROS(n,i,j)

!  Zooxanthellae release rate by host coral (individual cm-2 s-1)
!      Damage=exp(F_damage + 3.*F_ONOO)-1.   !Damage by ROS (F_damage) and peroxynitrite (F_ONOO)
!      Damage=exp(F_damage + F_ONOO)-1.   !Damage by ROS (F_damage) and peroxynitrite (F_ONOO)
      Damage = exp( 5.0d2 * F_damage ) - 1.0d0   !Damage by ROS (F_damage)
      F_Zexpul  = 5.0d-2 * Damage
...
# if defined CORAL_NUTRIENTS
     &            ,F_Nexcr                            & ! Organic N excretion controled by host coral (nmol cm-2 s-1)
     &            ,F_Nwaste                           & ! Waste organic N (nmol cm-2 s-1)
     &            ,F_Pexcr                            & ! Organic P excretion controled by host coral (nmol cm-2 s-1)
...
!   Respilation rate (nmolO2 cm-2 s-1) 
      Rc = Rmax(n)*f_temp                                            &
     &     *CORAL(ng)%QC(n,i,j)**4.0d0                               &  ! Hill equation
...
!      F_CO2=k_CO2i*(fCO2coe-fCO2cal)
      F_CO2=k_CO2i*(CORAL(ng)%cCO2aqcoe(n,i,j)-CORAL(ng)%cCO2aqcal(n,i,j))*rho_sw
      
!   Leak flux (nmol cm-2 s-1) 
      Fpp_TA =k_TA *(CORAL(ng)%TAcoe(n,i,j)-CORAL(ng)%TAcal(n,i,j))*rho_sw
      Fpp_DIC=k_DIC*(CORAL(ng)%DICcoe(n,i,j)-CORAL(ng)%DICcal(n,i,j))*rho_sw
      
!      Fpp_TA =k_TA *(TAamb-CORAL(ng)%TAcal(n,i,j))*rho_sw
!      Fpp_DIC=k_DIC*(DICamb-CORAL(ng)%DICcal(n,i,j))*rho_sw
...
#if defined CORAL_NONE_CO2_EQ
      Fpp_CO2aq=k_DIC*(CORAL(ng)%cCO2aqcoe(n,i,j)-CORAL(ng)%cCO2aqcal(n,i,j))*rho_sw
...
! Carbon isotope fluxes
      F_13CO2=k_CO2i/1.0007d0*(CORAL(ng)%c13CO2aqcoe(n,i,j)-CORAL(ng)%c13CO2aqcal(n,i,j))*rho_sw
      DI13Cuptake=Si *(DI13Camb-CORAL(ng)%DI13Ccoe(n,i,j))*rho_sw
      Fpp_DI13C=k_DIC*(CORAL(ng)%DI13Ccoe(n,i,j)-CORAL(ng)%DI13Ccal(n,i,j))*rho_sw
                    
# if defined CORAL_NONE_CO2_EQ
! Carbon isotope fluxes
      c13CO2aquptake=Si *(c13CO2aqamb-CORAL(ng)%c13CO2aqcoe(n,i,j))*rho_sw
      Fpp_13CO2aq=k_DIC*(CORAL(ng)%c13CO2aqcoe(n,i,j)-CORAL(ng)%c13CO2aqcal(n,i,j))*rho_sw
# endif
...
!-----------------------------------------------------------------------
      ...
      real(8), intent(in) :: TKamb                            !Temperature (K)
...
# if defined CORAL_NUTRIENTS
      real(8), intent(out) :: F_Nexcr                          ! Organic N excretion controled by host coral (nmol cm-2 s-1)
      real(8), intent(out) :: F_Nwaste                          ! Waste organic N (nmol cm-2 s-1)
      real(8), intent(out) :: F_Pexcr                          ! Organic P excretion controled by host coral (nmol cm-2 s-1)
      real(8), intent(out) :: F_Pwaste                          ! Waste organic P (nmol cm-2 s-1)
      real(8), intent(out) :: NO3_trans, NO2_trans, NH4_trans  ! Nutrients transport rate from host to Zooxanthellae (nmol cm-2 s-1)
      real(8), intent(out) :: PO4_trans                        ! Nutrients transport rate from host to Zooxanthellae (nmol cm-2 s-1)
      real(8), intent(out) :: F_ONOO                           ! Flux of peroxynitrite  (nmol cm-2 s-1)
# endif
...
!     &       *hcoe
      ...
      real(8), parameter :: lamb = 1000.d0 !*適当(未使用）

      real(8), parameter :: Pgmax(Ncl) = 0.80d-3 !1.0 (nmol cm-2 -s)=1.0/dens=1.0e-6 (nmol cell-1 s-1) =1.0e-3 (pmol cell-1 s-1)
      real(8), parameter :: Ik(Ncl)    = 275.d0    !(275 uEin m-2 s-1) * refference value
...
!----- Respitration constants ---------------------------
      ...
      real(8), parameter :: K_QC = 8.d0  ! (pmol cell-1) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*適当
      real(8), parameter :: K_DO = 5.d0  ! ca. 10 (umol kg-1) Shick et al. (1990) *Tuned value

!----- Zooxanthellar physica condition ---------------------------

      real(8), parameter :: V_zoox = 5.0d-10   ! Volume of zooxanthellae (cm3/cells)
                               ! ~500 um3 =5.e-10 cm3 (e.g. Jone 1997; Jones and Yellowlees 1997)

      real(8), parameter :: QC0 = 8.0d0  ! (pmolC/cell)  100 pg cell-1 ~ 10 pmol cell-1 = (Verde & McCloskey 1998)
# if defined CORAL_NUTRIENTS
...
!----- Nutrients related parameters -------------------
      real(8), parameter :: Vmax_NO3tra = 1.67d-6  ! (pmol cell-1 s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう 
      real(8), parameter :: Vmax_NO2tra = 1.5d-7  ! (pmol cell-1 s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
      real(8), parameter :: Vmax_NH4tra = 1.d-5  ! (pmol cell-1 s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
      real(8), parameter :: Vmax_PO4tra = 3.d-7  ! (pmol cell-1 s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう

      real(8), parameter :: K_NO3tra = 50.d0  ! (umol kg-1)  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
      real(8), parameter :: K_NO2tra = 10.d0  ! (umol kg-1)  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
      real(8), parameter :: K_NH4tra = 300.d0 ! (umol kg-1)  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう      
      real(8), parameter :: K_PO4tra = 300.d0 ! (umol kg-1)  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう      

      real(8), parameter :: k_NH4rel = 1.d-5  !(s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
      real(8), parameter :: k_NO3rel = 1.9d-8  !(s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
      real(8), parameter :: k_NO2rel = 1.d-5  !(s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
      real(8), parameter :: k_PO4rel = 1.d-5  !(s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう

      real(8), parameter :: Vmax_NO3red = 1.3d-6  ! (pmol cell-1 s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう 
      real(8), parameter :: Vmax_NO2red = 1.3d-6  ! (pmol cell-1 s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう

      real(8), parameter :: K_NO3red = 0.014d0  !(pmol cell-1)  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
      real(8), parameter :: K_NO2red = 0.0015d0 !(pmol cell-1)  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう

      real(8), parameter :: Vmax_Nass = 2.d-5  ! (pmol cell-1 s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう 
      real(8), parameter :: K_Nass = 1.d-1  !(pmol cell-1)  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう

      real(8), parameter :: Vmax_Pass = 5.d-7  ! (pmol cell-1 s-1) !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう 
      real(8), parameter :: K_Pass = 1.d-2  !(pmol cell-1)  !!!!!!!!!!!!!!!!!!!!!!!!!!てきとう
# endif

!----- Population dynamics parameters -------------------

      real(8), parameter :: u_max  = 2.0d-4 !Maximum reproduction rate (pmol cell-1 s-1)  !!!!!!!!!!!!!!!!!!適当!!!!!!!!!!!!!!
      
      real(8), parameter :: km_CNP = 1.d-8 ! (cell cm-2 s-1)  適当!!!!!!!!!!!!!!
      real(8), parameter :: am_CNP = 1.d0
      real(8), parameter :: km_ROS = 1.d-11 !                  適当!!!!!!!!!!!!!!
      real(8), parameter :: km_min = 1.d-14   ! (cell cm-2 s-1)  適当!!!!!!!!!!!!!!
...
!------ Organic matter excretion controled by host coral (pmol cell-1 s-1) -----

      F_Cexcr = 3.0d-5 * ZOOX(ng)%QC(n,i,j) 
...
# else
      C_repro = u_max*(1.0d0 - QC0/ZOOX(ng)%QC(n,i,j))        &
     &         *((2.0d6 - ZOOX(ng)%dens(n,i,j))/2.0d6)       !!! Logistic reproduction
      C_repro = max(C_repro, 0.0d0)
# endif
...
# if defined CORAL_NUTRIENTS
     ...
     &             )
!     &          +km_ROS * CORAL(ng)%ROS(n,i,j)                                !* ROS濃度依存（要検討!!!）  今のところゼロにしている
!     &          +km_min                                    !* 定数
# else
...
!-----Cell quota (pmol/cell) -----------------!!!!!単位変換チェック!!!!!!!!!!!!!!!!!!!!!!!!!!

      ZOOX(ng)%QC(n,i,j)=ZOOX(ng)%QC(n,i,j)                     &
     &              +(                                &
     &                 Pg-Rz                          &
     &                -F_Cexcr                        &   !(nmol cm-2 s-1)/(cell cm-2)*1.e3=(pmol cell-1 s-1)
     &                -C_repro                        &
     &               )*dt
...
!     &                +NH4uptake/ZOOX(ng)%dens(n,i,j)*1.e3  &  !(nmol cm-2 s-1)/(cell cm-2)*1.e3=(pmol cell-1 s-1)
     &                -F_Nexcr                         &  !(nmol cm-2 s-1)/(cell cm-2)*1.e3=(pmol cell-1 s-1)
     &                -N_repro                         &
     &               )*dt
...
!     &                 PO4uptake/ZOOX(ng)%dens(n,i,j)*1.e3   &  !(nmol cm-2 s-1)/(cell cm-2)*1.e3=(pmol cell-1 s-1)
     &                 P_assim                               &
     &                -F_Pexcr                               &   !(nmol cm-2 s-1)/(cell cm-2)*1.e3=(pmol cell-1 s-1)
     &                -P_repro                               &
     &               )*dt
...
!     ZOOX(ng)%dens ~ 1.e6 cells cm-2  (e.g. Higuchi et al., 2012)
     ...
     &               -Morta * ZOOX(ng)%dens(n,i,j)          & ! Death
     &               -F_Zexpul                              & ! Expulsion by host coral
     &              )*dt
...
!      F_ROS = 0.0072d0*exp(0.14d0*(TKamb-273.15))+0.0024d0*(PFDsurf) ! (fmol min-1 cell-1)
!  Estimated from combined data of Saragosti et al. (2010) and McGinty et al. (2012)
      F_ROS = 6.0d-7*exp(0.5d0*(TKamb-273.15))+0.0024d0*(PFDsurf) ! (fmol min-1 cell-1)

      F_ROS = F_ROS * ZOOX(ng)%dens(n,i,j)/60.0d0 * 1.0d-6   ! (nmol cm-2 s-1)
      
!----- peroxynitrite (ONOO-) flux (nmol cm-2 s-1)------------------------------------
!
!      F_ONOO=1.e0 * ZOOX(ng)%NO2(n,i,j)*CORAL(ng)%ROS(n,i,j)   ! (pmol cell-1 s-1)!!!!!!!!!!!!!!!!!!てきとう
!      
...
# if defined CORAL_TESTMODE
     ...
     &   ,F_Zexpul,Repro*ZOOX(ng)%dens(n,i,j),Morta*ZOOX(ng)%dens(n,i,j)      &
#  if defined CORAL_NUTRIENTS
     &   ,ZOOX(ng)%QN(n,i,j),ZOOX(ng)%QP(n,i,j)    &
     &   ,N_repro,P_repro   &
     &   ,ZOOX(ng)%NO3(n,i,j),ZOOX(ng)%NO2(n,i,j),ZOOX(ng)%NH4(n,i,j),F_ONOO &
...
!----- Outoput for coral internal system model (nmol cm-2 s-1)------------------------------------
      ...
      Pg=Pg*ZOOX(ng)%dens(n,i,j)*1.0d-3  !=(pmol cell-1 s-1)*(cell cm-1)*1.e-3=(nmol cm-2 s-1)
      Rz=Rz*ZOOX(ng)%dens(n,i,j)*1.0d-3  !=(pmol cell-1 s-1)*(cell cm-1)*1.e-3=(nmol cm-2 s-1)

      F_Cexcr = F_Cexcr*ZOOX(ng)%dens(n,i,j)*1.0d-3  !=(pmol cell-1 s-1)*(cell cm-1)*1.e-3=(nmol cm-2 s-1)
      F_Cwaste = (Death+F_Zexpul)*ZOOX(ng)%QC(n,i,j)*1.0d-3  !=(cell cm-2 s-1)*(pmol cell-1)*1.e-3=(nmol cm-2 s-1)

# if defined CORAL_NUTRIENTS
      F_Nexcr = F_Nexcr*ZOOX(ng)%dens(n,i,j)*1.0d-3  !=(pmol cell-1 s-1)*(cell cm-1)*1.e-3=(nmol cm-2 s-1)
      F_Pexcr = F_Pexcr*ZOOX(ng)%dens(n,i,j)*1.0d-3  !=(pmol cell-1 s-1)*(cell cm-1)*1.e-3=(nmol cm-2 s-1)
      F_Nwaste = (Death+F_Zexpul)*ZOOX(ng)%QN(n,i,j)*1.0d-3
      F_Pwaste = (Death+F_Zexpul)*ZOOX(ng)%QP(n,i,j)*1.0d-3
